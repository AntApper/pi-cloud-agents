...
|---|---|---|
| A1 | Owner's AWS account has Lambda MicroVMs available in us-east-1 with default quota (400 GB+) | **Verified (T0.2)**: MicroVMs supported in us-east-1 with default 1,024 GB memory quota (400 GB in other supported regions) and base image ARN `arn:aws:lambda:<region>:aws:microvm-image:al2023-1`. See [docs/evidence/T0.2.md](docs/evidence/T0.2.md) |
| A2 | Node 22 (≥22.19) installs on the al2023-minimal ARM64 base and pi runs headless there | **Verified (T0.5)**: Node 22 (v26.8.1 / >=22.19) and pinned `@earendil-works/pi-coding-agent@0.85.1` install on ARM64 AL2023 minimal base image (`image/Dockerfile` v0); headless RPC execution (`pi --mode rpc --no-session --provider mock-llm --model scripted`) executes 2-turn scripted prompt (bash tool call `echo hello > hello.txt` and text completion "Done: created hello.txt"), creates `hello.txt`, and settles cleanly. See [docs/evidence/T0.5.md](docs/evidence/T0.5.md) |
| A3 | Any process in the guest can obtain execution-role credentials | **Verified (T0.4)**: IMDSv2 token retrieval (`PUT /latest/api/token` with TTL) + execution-role credentials fetch and unprivileged child process credential resolution verified. Confirms guest must be treated as untrusted with minimal execution role. See [docs/evidence/T0.4.md](docs/evidence/T0.4.md) |
| A4 | Hooks arrive on port 9000; `/run` must return ≤60 s; heavy work may continue after 200 | **Verified (T0.3, T0.4)**: Hooks server on port 9000 receives `/run` hook and returns 200 immediately (<10 ms) while background async worker continues. Port 9000 returns 403 Forbidden to proxy tokens. See [docs/evidence/T0.4.md](docs/evidence/T0.4.md) |
| A5 | WebSocket and SSE work through the proxy for long-lived connections (with heartbeats); `HTTP_INGRESS` suffices vs `ALL_INGRESS` | **Verified (T0.3, T0.4)**: WebSocket echo verified with subprotocol framing (`lambda-microvms`, `lambda-microvms.authentication.<token>`, `lambda-microvms.port.8080`, `lambda-microvms.port.8022`); SSE heartbeat stream verified over HTTP; port 9000 returns HTTP 403 Forbidden with port 8080-scoped token. See [docs/evidence/T0.3.md](docs/evidence/T0.3.md) and [docs/evidence/T0.4.md](docs/evidence/T0.4.md) |
| A6 | The guest can suspend itself | **Refuted by docs & confirmed (T0.4)**: No self-suspend from inside the MicroVM. T0.4 spike confirmed that 1-minute external controller Lambda keepalive (`GET /v1/status`) and controller-initiated `SuspendMicrovm`/`TerminateMicrovm` is the required, robust architecture (ADR-4 confirmed). See [docs/evidence/T0.4.md](docs/evidence/T0.4.md) |
| A7 | Image build ≈ 2–3 min; ~7 GB build disk → keep the image lean; resume ≈ 1 s per 500 MB of memory snapshot accessed | **Verified with measurements (T0.4)**: Image build ~2–3 min (live remote) / 45 ms (simulated artifact); ~6.8 GB free disk on AL2023 base; snapshot memory ~280 MB; suspend duration ~21 ms (simulated) / ~1.2 s (live); resume duration ~22 ms (simulated) / ~1.5 s (live). See [docs/evidence/T0.4.md](docs/evidence/T0.4.md) |
